...
fn main() {
    ...
    coords.into_iter().for_each(|c| grid.plot_line(c));
...
impl Grid {
    ...
    fn plot_line(&mut self, coords: ((isize, isize), (isize, isize))) {
        let ((x1, y1), (x2, y2)) = coords;

        if x1 == x2 {
            let (min_y, max_y) = if y1 > y2 { (y2, y1) } else { (y1, y2) };

            for y in min_y..=max_y {
                self[x1.try_into().unwrap()][y as usize] += 1;
            }
        } else if y1 == y2 {
            let (min_x, max_x) = if x1 > x2 { (x2, x1) } else { (x1, x2) };
            for x in min_x..=max_x {
                self[x as usize][y1 as usize] += 1;
            }
        } else {
            let direction = if (x1 < x2) == (y1 < y2) { 1 } else { -1 };
            for i in 0.min(x2 - x1)..=0.max(x2 - x1) {
                self[(x1 + i) as usize][(y1 + i * direction) as usize] += 1;
            }
        }
    }
}
